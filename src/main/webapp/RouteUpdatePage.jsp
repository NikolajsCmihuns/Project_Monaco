...
<div id="container">
    <div id="menu">
        <form id="fmenu" name="fmenu" action="${pageContext.request.contextPath}/route_update" method="POST">
            ...
            <select id="helperCountry" name="helperCountry" class="select_width" onchange="getCoordinates()">
                <%
                    ...
                    String routeToUpdateCountry = new String(Base64.getDecoder().decode(request.getParameter("routeToUpdateCountry")));
                    String countryKey = "";
                    Iterator<HelperCountry> iteratorCountries = allCountries.iterator();
                    while (iteratorCountries.hasNext() && "".equals(countryKey)) {
                        HelperCountry helperCountry = iteratorCountries.next();
                        if (helperCountry.getLongName().equals(routeToUpdateCountry)) {
                            countryKey = helperCountry.getShortName();
                        }
                    }
                %>
                <option value="<%=countryKey%>"><%=routeToUpdateCountry%>
                </option>
                <%
                    iteratorCountries = allCountries.iterator();
                    while (iteratorCountries.hasNext()) {
                        HelperCountry helperCountry = iteratorCountries.next();
                        if (!helperCountry.getShortName().equals(countryKey)) {
                %>
            ...
            <%
                String routeToUpdateCity = new String(Base64.getDecoder().decode(request.getParameter("routeToUpdateCity")));
            %>
            <input type="text" id="city" name="city" value="<%=routeToUpdateCity%>" class="tag_width"/>
            <%
                String routeToUpdateName = new String(Base64.getDecoder().decode(request.getParameter("routeToUpdateName")));
            %>
            <input type="text" id="routeName" name="routeName" class="tag_width"
                   placeholder="Enter route name" required value="<%=routeToUpdateName%>"/>
            <select required id="tag" name="tag" class="select_width">
                <%
                    ...
                    String tagText = "";
                    String decodedTag = new String(Base64.getDecoder().decode(request.getParameter("routeToUpdateTagId")));
                    while (iteratorTags.hasNext() && "".equals(tagText)) {
                        HelperTags tag = iteratorTags.next();
                        if (Integer.parseInt(decodedTag) == tag.getTagId()) {
                            tagText = tag.getTagName();
                        }
                    }
                %>
                <option value="<%=decodedTag%>"><%=tagText%>
                </option>
                <%
                    helperTagsList = routeMetaInfo.get("tags");
                    iteratorTags = helperTagsList.iterator();
            ...
            <%
                String updateRouteDistance = new String(Base64.getDecoder().decode(request.getParameter("routeToUpdateDistance")));
            %>
            <input type="text" readonly class="distance" id="routeDistance" name="routeDistance"
                   value="<%=updateRouteDistance%>"/>&nbsp;km
            <input type="submit" value="Save">
            <%
                String route = new String(Base64.getDecoder().decode(request.getParameter("routeToUpdateRoute")));
            %>
            <input type="hidden" id="routeToUpdate" name="routeToUpdate" value="<%=route%>"/>
            <%
                String routeId = new String(Base64.getDecoder().decode(request.getParameter("routeToUpdateId")));
            %>
...
<script type="text/javascript">
    ...
    var map;
    var cities;
    var currentCountry = document.getElementById("helperCountry").value;
    var places;
    ...
    function addResult(result, i) {
        ...
        // if a place already exists in the array we want to mark it
        var currentPlace = result.geometry.location.lat() + "|" + result.geometry.location.lng();
        var marked = false;
        var i = 0;
        var placesInTheRoute = document.getElementById("routeToUpdate").value.split("|");
        while (i < placesInTheRoute.length && marked == false) {
            var placeNode = placesInTheRoute[i].replace(",", "|");
            var isPlaceInTheRoute = currentPlace.localeCompare(placeNode);
            if (isPlaceInTheRoute == 0) {
                tr.style.backgroundColor = '#00FF00';
                marked = true;
                routeNodes.push(currentPlace);
            }
            i++;
        }
